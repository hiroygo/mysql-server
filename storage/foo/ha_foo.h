...
/** @file ha_foo.h
  ...
  Reminder: The foo storage engine implements all methods that are
...
/** @brief
  Foo_share is a class that will be shared among all open handlers.
  This foo implements the minimum of what you will probably need.
*/
class Foo_share : public Handler_share {
 public:
  THR_LOCK lock;
  Foo_share();
  ~Foo_share() override { thr_lock_delete(&lock); }
};
...
class ha_foo : public handler {
 ...
 public:
   ...
   */
  const char *table_type() const override { return "FOO"; }
